...
export function CreateProductForm({ categories, players }: CreateProductFormProps) {
    ...
    const [demographic, setDemographic] = useState<Demographic>(Demographic.UNISEX);
    ...
    const [variants, setVariants] = useState<VariantItem[]>(() => [
        { id: crypto.randomUUID(), size: "M", stock: 0, sku: "" }
    ]);
    ...
    const createProductWithData = createProduct.bind(null, boundData);
    const [state, actionFn, isPending] = useActionState(createProductWithData, undefined);
        ...
        <form action={actionFn} className="space-y-6">
            <Tabs defaultValue="basic" className="w-full">
                <TabsList className="flex flex-wrap w-full max-w-fit mb-6 justify-start h-auto gap-1">
                    <TabsTrigger value="basic" className="data-[state=active]:bg-muted data-[state=active]:shadow-none border border-transparent data-[state=active]:border-border rounded-md px-4 py-2">
                        Базова інформація
                    </TabsTrigger>
                    <TabsTrigger value="price" className="data-[state=active]:bg-muted data-[state=active]:shadow-none border border-transparent data-[state=active]:border-border rounded-md px-4 py-2">
                        Ціни та Статус
                    </TabsTrigger>
                    <TabsTrigger value="attributes" className="data-[state=active]:bg-muted data-[state=active]:shadow-none border border-transparent data-[state=active]:border-border rounded-md px-4 py-2">
                        Атрибути та Гравці
                    </TabsTrigger>
                    <TabsTrigger value="variants" className="data-[state=active]:bg-muted data-[state=active]:shadow-none border border-transparent data-[state=active]:border-border rounded-md px-4 py-2">
                        Розміри ({variants.length})
                    </TabsTrigger>
                    <TabsTrigger value="media" className="data-[state=active]:bg-muted data-[state=active]:shadow-none border border-transparent data-[state=active]:border-border rounded-md px-4 py-2">
                        Фотографії
                    </TabsTrigger>
                </TabsList>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 items-start">
                            ...
                            <Tabs defaultValue="uk" className="w-full border rounded-lg bg-card p-4 sm:p-6 shadow-sm">
                                ...
                                <TabsContent value="uk" className="space-y-4 outline-none">
                                    <div className="space-y-2">
                                        <Label htmlFor="name_uk" className="text-base font-semibold">Назва товару <span className="text-red-500">*</span></Label>
                                        <Input
                                            id="name_uk"
                                            value={nameUk}
                                    ...
                                    <div className="space-y-2 pt-2">
                                        <Label className="text-base font-semibold">Опис товару <span className="text-red-500">*</span></Label>
                                        <div className="w-full">
                                            <RichTextEditor value={descUk} onChange={setDescUk} disabled={isPending} />
                                        </div>
                                        {state?.errors?.description_uk && <p className="text-red-500 text-sm">{state.errors.description_uk[0]}</p>}
                                    </div>
                                </TabsContent>
                                <TabsContent value="en" className="space-y-4 outline-none">
                                    <div className="space-y-2">
                                        <Label htmlFor="name_en" className="text-base font-semibold">Назва товару (Англійська)<span className="text-red-500">*</span></Label>
                                        <Input
                                            id="name_en"
                                            value={nameEn}
                                    ...
                                    <div className="space-y-2 pt-2">
                                        <Label className="text-base font-semibold">Опис товару (Англійська)<span className="text-red-500">*</span></Label>
                                        <div className="w-full">
                                            <RichTextEditor value={descEn} onChange={setDescEn} disabled={isPending} />
                                        </div>
                                        {state?.errors?.description_en && <p className="text-red-500 text-sm">{state.errors.description_en[0]}</p>}
                                    </div>
                                </TabsContent>
                            </Tabs>
                        </div>
                        <div className="xl:col-span-1 space-y-6 h-full flex flex-col">
                            <Card className="shadow-none border-border/50 h-full">
                                <CardHeader><CardTitle className="text-lg">Класифікація</CardTitle></CardHeader>
                                <CardContent className="flex flex-col space-y-6">
                                    <div className="space-y-2">
                                        <Label>Категорія <span className="text-red-500">*</span></Label>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Select value={demographic} onValueChange={v => setDemographic(v as Demographic)} disabled={isPending}>
                ...
                <TabsContent value="price" className="outline-none">
                    <Card className="shadow-none border-border/50">
                        <CardHeader><CardTitle className="text-lg">Ціноутворення</CardTitle></CardHeader>
                        <CardContent className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl">
                                <div className="space-y-2">
                                    <Label>Звичайна ціна (₴) <span className="text-red-500">*</span></Label>
                                    <Input type="number" step="0.01" min={0} value={price} onChange={e => setPrice(e.target.value ? Number(e.target.value) : "")} disabled={isPending} />
                                    {state?.errors?.price && <p className="text-red-500 text-xs">{state.errors.price[0]}</p>}
                                </div>
                                <div className="space-y-2">
                                    <Label>Акційна ціна (₴)</Label>
                                    <Input type="number" step="0.01" min={0} value={salePrice} onChange={e => setSalePrice(e.target.value ? Number(e.target.value) : "")} disabled={isPending || !isOnSale} />
                                    {state?.errors?.salePrice && <p className="text-red-500 text-xs">{state.errors.salePrice[0]}</p>}
                                </div>
                            </div>
                            <div className="space-y-4 pt-4 border-t max-w-3xl">
                                ...
                                <div className="flex flex-row items-center justify-between rounded-lg border p-3 bg-muted/10">
                                    <div>
                                        ...
                                        <p className="text-xs text-muted-foreground">Товар буде виділений на головній сторінці магазину.</p>
                                    </div>
                ...
                <TabsContent value="attributes" className="outline-none space-y-6">
                    <Card className="shadow-none border-border/50 w-full">
                        <CardHeader><CardTitle className="text-lg">Характеристики одягу (Опціонально)</CardTitle></CardHeader>
                        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input value={seasonYear} onChange={e => setSeasonYear(e.target.value)} disabled={isPending} placeholder="напр. 2025/2026" />
                            </div>
                            <div className="space-y-2">
                                <Label>Тип матчу</Label>
                                <Input value={matchType} onChange={e => setMatchType(e.target.value)} disabled={isPending} placeholder="напр. Домашня, Виїзна" />
                            </div>
                        </CardContent>
                    </Card>
                    <Card className="shadow-none border-border/50 w-full">
                ...
                <TabsContent value="variants" className="outline-none">
                    <Card className="shadow-none border-border/50 w-full">
                        <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4">
                            ...
                            <Button type="button" onClick={addVariant} variant="secondary" size="sm" disabled={isPending}>
                                <Plus className="w-4 h-4 mr-2" /> Додати розмір
                            </Button>
                        </CardHeader>
                        <CardContent>
                            {state?.errors?.variants && <p className="text-red-500 text-sm mb-4">{state.errors.variants[0]}</p>}
                            <div className="space-y-3">
                                {variants.map((variant, idx) => (
                                    <div key={variant.id} className="flex flex-col sm:flex-row gap-4 items-end sm:items-center p-4 border rounded-lg bg-muted/10 relative">
                                        <div className="space-y-1 w-full sm:w-1/3">
                                            <Label>Розмір <span className="text-red-500">*</span></Label>
                                            <Select
                                                value={variant.size}
                                                onValueChange={val => updateVariant(idx, 'size', val)}
                                                disabled={isPending}
                                            >
                                                <SelectTrigger><SelectValue placeholder="Оберіть розмір" /></SelectTrigger>
                                                <SelectContent>
                                                    {STANDARD_SIZES.map(sz => {
                                                        const isAlreadySelected = variants.some((v, i) => v.size === sz && i !== idx);
                                                        return (
                                        ...
                                        <div className="space-y-1 w-full sm:w-1/3">
                                            <Label>Залишок (шт.) <span className="text-red-500">*</span></Label>
                                            <Input type="number" min={0} value={variant.stock} onChange={e => updateVariant(idx, 'stock', Number(e.target.value))} disabled={isPending} />
                                        </div>
                                        <div className="space-y-1 w-full sm:w-1/3">
                                            <Label>Артикул / SKU (Опц.)</Label>
                                            <Input value={variant.sku} onChange={e => updateVariant(idx, 'sku', e.target.value)} placeholder="FC-TS-M-01" disabled={isPending} />
                                        </div>

                                        <Button type="button" variant="ghost" className="text-destructive hover:bg-destructive/10 sm:mb-0.5 shrink-0" size="icon" onClick={() => removeVariant(idx)} disabled={isPending || variants.length === 1}>
                ...
                <TabsContent value="media" className="outline-none">
                    <Card className="shadow-none border-border/50 w-full">
                        <CardHeader>
                            ...
                            <CardDescription>Завантажте зображення (макс. 5 шт). Перше фото буде використано як обкладинка.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            {mediaUrls.length > 0 && (
                                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3">
                                    {mediaUrls.map((url, idx) => (
                                        <div key={idx} className="relative group rounded-md overflow-hidden aspect-square border-2 border-border/50 hover:border-border transition-all">
                                            <Image src={url} alt={`Product media ${idx}`} fill className="object-cover" unoptimized />
                                            {idx === 0 && (
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-end border-t pt-6 mt-6 w-full">
                ...
                <Button type="submit" className="w-full sm:w-auto min-w-48" disabled={isPending}>
                    {isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Збереження...</> : "Створити товар"}
                </Button>
            </div>
        </form>
    );
