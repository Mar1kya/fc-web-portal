...
import { restoreGallery, hardDeleteGallery } from "@/actions/gallery";
import { type ArchivedGalleryWithRelations } from "./archive-columns";
import { getTranslation } from "@/lib/utils/get-translation";

export function GalleryArchiveActions({ gallery }: { gallery: ArchivedGalleryWithRelations }) {
    ...
    const handleRestore = () => {
        startTransition(async () => {
            const result = await restoreGallery(gallery.id);
            if (result?.success) {
                toast.success(result.message);
            } else {
                toast.error(result?.message || "Не вдалося відновити галерею");
            }
        });
    };
    ...
    const handleHardDelete = () => {
        startTransition(async () => {
            const result = await hardDeleteGallery(gallery.id);
            if (result?.success) {
                toast.success(result.message);
            ...
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        Ви збираєтеся назавжди видалити галерею <strong>&quot;{title}&quot;</strong>.
                        Цю дію неможливо скасувати. Всі фото, прив&apos;язані до цієї галереї, будуть знищені!
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
