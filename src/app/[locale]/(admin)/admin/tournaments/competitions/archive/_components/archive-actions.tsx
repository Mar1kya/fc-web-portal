...
import { restoreTournament, hardDeleteTournament } from "@/actions/tournament"
import { getTranslation } from "@/lib/utils/get-translation"
import { TournamentWithRelations } from "../../_components/columns"

export function ArchiveActions({ tournament }: { tournament: TournamentWithRelations }) {
    ...
    const handleRestore = () => {
        startTransition(async () => {
            const result = await restoreTournament(tournament.id);
            if (result?.success) {
                toast.success(result.message);
            } else {
                toast.error(result?.message || "Не вдалося відновити турнір");
            }
        });
    };
    ...
    const handleHardDelete = () => {
        startTransition(async () => {
            const result = await hardDeleteTournament(tournament.id);
            if (result?.success) {
                toast.success(result.message);
            ...
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        Ви збираєтеся назавжди видалити турнір <strong>&quot;{name}&quot;</strong>.
                        Цю дію неможливо скасувати. Усі пов&apos;язані з ним матчі та турнірні таблиці будуть видалені з бази даних!
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
